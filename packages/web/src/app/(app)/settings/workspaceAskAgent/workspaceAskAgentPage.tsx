...
import { AlertTriangleIcon, CableIcon, Loader2, MinusIcon, PlusIcon } from "lucide-react";
...
export function WorkspaceAskAgentPage() {
    ...
    const handleDelete = async (serverId: string) => {
        ...
        try {
            ...
            if (isServiceError(result)) {
                toast({ title: "Error", description: `Failed to delete connector: ${result.message}`, variant: "destructive" });
                return;
            }

            await invalidateMcpConfigurationQueries(queryClient);
        } catch (error) {
            toast({ title: "Error", description: `Failed to delete connector: ${error}`, variant: "destructive" });
        } finally {
            ...
            <div className="space-y-3">
                ...
                <Card>
                    <CardContent className="p-0 divide-y">
                        <div className="flex items-center justify-between gap-4 p-5">
                            <div>
                                <p className="font-medium">Saved connector connections</p>
                                <p className="text-sm text-muted-foreground">
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between gap-4">
                        <div>
                            <CardTitle>{isLoading ? "Allowed connectors" : `${servers.length} allowed ${pluralize(servers.length, "connector")}`}</CardTitle>
                            <CardDescription>
                                {isOAuthUnavailable
                        ...
                        {canCreateConnectors ? (
                            ...
                            <Button size="icon" variant="ghost" disabled aria-label="Add connector unavailable">
                    ...
                    <CardContent>
                        {isLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <p className="text-sm font-medium text-foreground mb-1">No connectors configured yet</p>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    ...
                                                    <AlertDialogDescription>
                                                        Are you sure you want to remove <span className="font-semibold text-foreground">{server.name || server.serverUrl}</span>? Workspace members will lose access and stored credentials for this connector.
                                                    </AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
