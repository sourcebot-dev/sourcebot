...
// blobs the model can natively accept (by modality or document type) are loaded,
// and only when linked to this chat (mirroring the serving route's chat-derived
// access), so a text-only model resolves nothing here and the builder leaves a
// marker instead.
...
export const createMessageStream = async ({
    ...
    // all user turns up-front, reading from the StorageBackend. `lastUserIndex`
    // still gates the (once-only) inlining of text attachments below.
