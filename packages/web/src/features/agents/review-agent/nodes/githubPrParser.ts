import {
    sourcebot_pr_payload,
    sourcebot_file_diff,
    sourcebot_diff,
} from "@/features/agents/review-agent/types";
...
export const githubPrParser = async (
    octokit: Octokit,
    ...
    const sourcebotFileDiffs: (sourcebot_file_diff | null)[] = parsedDiff.map(
        (file) => {
            if (!file.from || !file.to) {
                logger.debug(
                    `Skipping file due to missing from (${file.from}) or to (${file.to})`,
                );
                return null;
            }
    ...
    const filteredSourcebotFileDiffs: sourcebot_file_diff[] =
        sourcebotFileDiffs.filter(
            (file) => file !== null,
        ) as sourcebot_file_diff[];
