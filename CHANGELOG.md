...
- [EE] Fixed account-driven permission sync jobs failing when OAuth tokens expire between user visits by moving token refresh into the backend sync flow. A `tokenRefreshErrorMessage` field is now set on the account and surfaced in the UI when refresh fails, prompting the user to re-authenticate. [#1000](https://github.com/sourcebot-dev/sourcebot/pull/1000)
