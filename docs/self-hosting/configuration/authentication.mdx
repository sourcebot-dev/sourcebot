...
The first account that's registered on a Sourcebot deployment is made the owner. All other users who register must be approved by the owner. If you have an [enterprise license](/self-hosting/license-key), you can
enable [AUTH_EE_ENABLE_JIT_PROVISIONING](/self-hosting/configuration/authentication#enterprise-authentication-providers) to have Sourcebot accounts automatically created and approved on registration.
...
they register for the first time, set the `AUTH_EE_ENABLE_JIT_PROVISIONING` to `true`. 
