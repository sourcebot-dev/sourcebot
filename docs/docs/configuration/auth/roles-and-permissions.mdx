...
| `Member` | Members have access and to use all standard features, such as code search, ask, mcp, etc., as well as their account settings. Note that a member will only be able to view repositories they have access to if [permission syncing](/docs/features/permission-syncing) is enabled. Members **cannot access** organization-level administration pages.  |
| `Guest` | Guests are users that access Sourcebot without a account when [anonymous access](/docs/configuration/auth/access-settings#anonymous-access) is enabled. Guests can use some features, such as code search and ask, with additional limitations like not having chat history. Guests **cannot access** organization-level administration pages. |
