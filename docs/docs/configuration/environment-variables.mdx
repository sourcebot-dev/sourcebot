...
| `DEFAULT_HOME_VIEW_PAGE` | `search` | <p>Sets the default home page when no `sb.home-view` cookie preference is set. Use `ask` to default to Ask Sourcebot or `search` to default to Code Search.</p> |
