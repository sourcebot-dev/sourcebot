...
<AccordionGroup>
    <Accordion title="Declarative configuration file">
        This is only supported when self-hosting, and is the default mechanism to define connections. Connections are defined in a [JSON file](/docs/configuration/declarative-config)
    ...
    <Accordion title="UI connection management">
        This is the only way to define connections when using Sourcebot Cloud, and can be configured when self-hosting by enabling [authentication](/docs/configuration/authentications).
