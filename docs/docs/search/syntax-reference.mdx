...
| `context:` | Filter results to a predefined [search context](/self-hosting/search/search-contexts). | `context:web` - Filter results to the web context<br/>`-context:pipelines` - Ignore results from the pipelines context |