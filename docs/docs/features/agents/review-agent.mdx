...
Set **Context files** in the agent config form to a comma or space separated list of paths relative to the repository root (e.g. `AGENTS.md .sourcebot/review.md`). Files that do not exist in the repository are silently ignored. The files are fetched once per PR from the repository's default branch and included in the context for every per-file review.
...
| `REVIEW_AGENT_REVIEW_COMMAND` | `review` | Comment command that triggers a manual review. You can override this in an agent config. |
